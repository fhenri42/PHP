...
MON_HTML;
        ...
        if ($_POST['submit'] == NULL || $_POST["login"]== NULL || $_POST["oldpw"] == NULL || $_POST["newpw"] == NULL)
        {
echo <<<MON_HTML
        <html>
...
MON_HTML;
        ...
        if (!isset($found_key) || $aut[$found_key]['pass'] !== hash("whirlpool", $_POST['oldpw']))
            {
echo <<<MON_HTML
        <html>
        Vous n'aver pas rentrer les bonne infotmation! 
...
return;
        ...
        $aut[$found_key]["pass"] = hash("whirlpool", $_POST["newpw"]);
        file_put_contents("../private/info_user", serialize($aut));
        echo <<<MON_HTML
        <html>
        votres mot de passe vien d'etre modifier! 
        <br />
        <br/>
        <A HREF= "../home.html" style="margin-right: 15px"> 'HOME'</A>
        </html>
